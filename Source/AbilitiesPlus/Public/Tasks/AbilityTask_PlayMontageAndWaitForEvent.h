 ...
 */
UCLASS()
class ABILITIESPLUS_API UAbilityTask_PlayMontageAndWaitForEvent
	: public UAbilityTask
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Abilities Plus|Ability|Tasks", meta = (HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "TRUE"))
...
private:
	...
	TComponent* GetTargetAbilitySystemComponent();

	template <>
	UAbilitySystemComponent* GetTargetAbilitySystemComponent<UAbilitySystemComponent>();
...
template <typename TComponent>
TComponent* UAbilityTask_PlayMontageAndWaitForEvent::GetTargetAbilitySystemComponent()
{
	static_assert(!TPointerIsConvertibleFromTo<TComponent*, UAbilitySystemComponent>::Value, "TComponent must be a child of UAbilitySystemComponent.");

	return Cast<TComponent>(AbilitySystemComponent);
}
